...
def standardize_column_name(col_name, is_metadata=False):
    """
    Standardizes column names to Title Case, but preserves specific metadata column names.
    
    Args:
        col_name: The column name to standardize
        is_metadata: If True, preserve the exact casing for metadata columns
    """
    if is_metadata:
        # Preserve exact casing for metadata columns
        return col_name
    
    # For data columns, use Title Case
    return str(col_name).strip().title()
    ...
    for filepath in all_files:
        ...
        if file_type == 'PKRV':
            ...
            if df is not None:
                ...
                df.columns = [standardize_column_name(c, is_metadata=False) for c in df.columns]
                
                # Add metadata columns (preserve exact casing)
            ...
            df = read_data_with_header(filepath, header_index)
            if df is not None:
                # Standardize data column names to Title Case
                df.columns = [standardize_column_name(c, is_metadata=False) for c in df.columns]
                
                # Add metadata columns with EXACT casing to match FastAPI app expectations
                df['Report_Date'] = metadata['date']
                df['Source_Filepath'] = filepath
        ...
        for i, df in enumerate(pkfrv_data_to_load, 1):
            for col in sorted_columns:
                if col not in df.columns:
                    df[col] = None  # Use None instead of 0 for missing data
